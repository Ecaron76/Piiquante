...
exports.likeDislike = ('/:id',(req, res, next) => {
  Sauce.findOne({_id : req.params.id})
    .then(sauce => {
      if (req.body.like === 1) {
        if (sauce.usersLiked.includes(req.body.userId)) 
          {
            res.status(401).json({error: 'Vous avez déjà aimé cette sauce'});
          }else{
          ...
          if (sauce.usersDisliked.includes(req.body._userId)) {
            res.status(401).json({message : "Vous n'aimez déjà pas cette sauce"})
          }else{
